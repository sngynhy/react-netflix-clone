...
import {Wrapper, Container, Nav, NavItem, Account, SearchBox, styles} from 'styles/HeaderStyle'
...
function Header () {
    ...
    function search (e) {
        ...
        // if ((e.keyCode === 13 || !!openSearchInput) && searchKeyword.trim().length !== 0) {
    ...
    return (
        <Wrapper id="header">
            <Container>
                {/* 좌측 넷플릭스 로고 */}
                <Link to="/"><img loading="lazy" src={logo} onClick={() => setCurrent(0)} style={{width: 120, verticalAlign:'middle'}} alt="logo"/></Link>
                            ...
                            <NavItem key={i}>
                                <Link to={el.path}>
                                    <div onClick={() => setCurrent(i)}>
                                        {/* <span className="categoryName" onMouseOver="" style={{borderBottom: current === i ? '2px solid red' : 'none'}}>{el.name}</span> */}
                                        <CategoryText selected={current === i}>{el.name}</CategoryText>
                                    </div>
                                </Link>
                            </NavItem>
                    )})}
                        ...
                        <label htmlFor="searchInput" style={{height: 'inherit'}}><IoSearch onClick={(e) => openSearchInput ? search(e) : setOpenSearchInput(true)} style={styles.searchIcon}/></label>
                            {
                                openSearchInput &&
                                    <>
                                        ...
                                        <IoCloseSharp onClick={() => setSearchKeyword('')} style={styles.searchIcon}/>
                                    </>
                            }
                    </SearchBox>
